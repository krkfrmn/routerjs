...
## Usage
Using routerjs is easy; it is designed to be managed by a cascade of config files whose ultimate configuration is specified by the environment (dev/test/prod etc.). A config file called _default.json is always loaded and then values may be overridden (or added) by a second config file matching the execution environment (<env>.json).
